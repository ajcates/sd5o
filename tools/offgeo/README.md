...
Not yet built: the two Census (`ADDRFEAT`/`FEATNAMES`) readers, and `OFF-103`'s full range/join-quality profiling pass (both readers' sentinel/implausibility counters are early signal toward that, not the full profiling item — that still needs the actual SanGIS-roads-to-address-points join by `ROADSEGID` run end-to-end over both readers' real output, not just each source's own internal counts).
