...
**A note on run-to-run variance, disclosed rather than smoothed over:** the 4,576.67 μs mean above came from a single run. Repeating the identical benchmark (same bytes, same sample, same host) five more times produced means of 2,902.71, 2,815.53, 3,023.35, and 2,906.96 μs — a real, ~1.6x spread, most likely JIT-state and system-load noise rather than anything wrong with the methodology. Treat the table above as "tens of x, not hundreds" rather than a precise multiplier; the qualitative conclusion (custom format meaningfully slower than SQLite in real JS, but the Python number overstated how much) holds across every run.
...
## Phase R1, Group D continued — attempting to close the JS decode-speed gap

```sh
node tools/offgeo/prototype-js/benchmark.mjs   # now runs baseline and optimized decoders back-to-back
```

Given the custom format's naive JS decoder came out ~30x slower than sql.js above, the natural next question: is that gap fundamental, or just an unoptimized implementation? Profiling `packformat.mjs`'s decode against a real, representative block (488 records, 19,910 geometry points) found the answer isn't uniform — geometry decode dominates (~2.7 ms of a ~3.9 ms total warmed-up decode), and its cost traces to two concrete, fixable things: `readUvarint`/`readSvarint` returning a fresh `[value, offset]` array on every single call (tens of thousands of small allocations per block), and each polyline vertex getting its own 2-element JS array instead of a shared contiguous buffer.

`tools/offgeo/prototype-js/packformat-fast.mjs` fixes both — varint decoding is inlined directly into each loop (no function call, no temp-array allocation) and each geometry entry decodes into one flat `Float64Array` instead of an array of pairs. Correctness is verified two ways before trusting the speed number: a dedicated test (`packformat-fast.test.mjs`) confirms every field matches `packformat.mjs`'s output on a hand-built block, and a separate real-data cross-check decoded every 7th real block (23,808 records) through both decoders and found **zero mismatches** on any field, including geometry.

**Real result:** the optimized decoder is consistently **~1.8–2.0x faster** than the direct port (e.g. 2,902.71 → 1,553.93 μs mean in one paired run) — a real, verified, worthwhile improvement, achieved purely by removing allocation overhead, no format change. But it **does not close the gap to sql.js**: at ~1,500–1,630 μs mean vs. sql.js's 150.72 μs, the optimized custom decoder is still roughly **10x slower**, not competitive.

**Why the remaining gap is structural, not just more allocation to remove** (this reasoning surfaced while working through the profiling data, credited here rather than presented as this document's own first idea): the block-partitioned custom format's index is only one level deep — `street key → block id` is O(1), but everything *inside* a block is an unindexed linear array, so a lookup pays for decoding the entire block (up to 500 records, tens of thousands of geometry points) even though it only needs one record's worth of data. SQLite's B-tree index, by contrast, gives O(log n) access all the way down to the matching row — that structural difference, not implementation sloppiness, is most of what's left after the allocation fix. Closing it for real would need an index *within* each block (e.g. sorted keys + binary search, or separating small fixed-width "key + range" fields from large variable-length geometry so a lookup only ever decodes the cheap part until it finds a match) — a real design change, not attempted here.

**Where this leaves `OFF-109`, updated:** the custom format's speed disadvantage against SQLite-via-sql.js narrows with real optimization work (from ~30x to ~10x here) but does not disappear, and the reason it doesn't is a genuine structural gap in this prototype's block design, not merely unoptimized code — worth knowing before assuming "a faster JS implementation" alone would settle the decision. Both the size advantage (custom format, `OFF-104`) and the now-better-understood speed disadvantage (SQLite, this section) remain real and simultaneously true; `OFF-109`'s actual recorded decision is still open.

Artifacts: `tools/offgeo/prototype-js/packformat-fast.mjs` (+ `packformat-fast.test.mjs`), `benchmark.mjs` extended to run both decoders. `build/offgeo-sources/r1-js-decode-benchmark-report.json` now reports both `baseline` and `optimized` results plus the measured speedup factor.
